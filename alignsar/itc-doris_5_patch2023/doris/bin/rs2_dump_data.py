...
cmd = '%s %s -ot %s -of %s -co %s %s' % (
    gdalCall, inputFileName, outputDataType, outputDataFormat, outputInterleave, outputFileName + '.noflip'
)

if outputWinFirstPix is not None:
    # xoff yoff xsize ysize
    cmd = cmd + (' -srcwin %s %s %s %s' % (
        outputWinFirstPix - 1,
        outputWinFirstLine - 1,
        outputWinLastPix - outputWinFirstPix + 1,
        outputWinLastLine - outputWinFirstLine + 1
    ))
    print(cmd)
...
cpxfcall = 'cpxfiddle -q normal -o short '  # e.g. cpxfiddle -w 3788 -f ci2 -q normal -o short -m Y in.noflip > out

if PASS == 'Ascending':
    cmd = cpxfcall + ('-w %s -f %s -m %s %s > %s ' % (
        outputWinLastPix - outputWinFirstPix + 1, outputDataType2, 'Y',
        outputFileName + '.noflip', outputFileName))
    print('INFO     : %s ' % cmd)
else:
    cmd = cpxfcall + ('-w %s -f %s -m %s %s > %s ' % (
        outputWinLastPix - outputWinFirstPix + 1, outputDataType2, 'X',
        outputFileName + '.noflip', outputFileName))
    print('INFO     : %s ' % cmd)
...
if resFile is not None:
    ...
    for line in headerFileStream:
        pair = line.split()
        if len(pair) > 1:
            # set IMAGE_LINES and LINE_SAMPLES
            vars()[pair[0]] = pair[2]
    ...
    sourceText  = "crop:\t\t\t0"
    replaceText = "crop:\t\t\t1"
    with open(resFile, 'r') as inputStream:
        textStream = inputStream.read()
    with open(resFile, 'w') as outputStream:
