...
if len(sys.argv) == 3:
    ...
    outputWinLastLine  = int(sys.argv[4])        # Lastline --> ysize (later)
    outputWinFirstPix  = int(sys.argv[5]) - 1    # gdal srcwin starting at 0
    outputWinLastPix   = int(sys.argv[6])        # Lastpix  --> xsize (later)
...
if outputWinFirstPix is not None:
    cmd = cmd + (' -srcwin %s %s %s %s' % (
        outputWinFirstPix,
        outputWinFirstLine,
        outputWinLastPix - outputWinFirstPix,
        outputWinLastLine - outputWinFirstLine
    ))
    # print(cmd)
